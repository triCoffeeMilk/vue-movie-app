...
describe('routes/Movie.vue', () => {
  ...
  test('최초 접속한 URL의 파라미터를 확인합니다', () => {
    expect(wrapper.vm.$route.params.imdbID).toBe('tt9376612')
  })
  ...
  test('정상적인 이미지 주소가 아닌 경우 빈 문자열을 반환합니다', () => {
    expect(wrapper.vm.requestDiffSizeImage()).toContain('')
    expect(wrapper.vm.requestDiffSizeImage('N/A')).toContain('')
  })
